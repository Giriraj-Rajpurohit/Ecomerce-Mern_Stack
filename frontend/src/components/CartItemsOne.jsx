        ...
        <table className='w-full mx-auto'>
            <thead>
                <tr className='bg-slate-900/10 regular-18 sm:regular-22 text-start py-12'>
                    <th className='p-1 p-y-2'>Products</th>
                    <th className='p-1 p-y-2'>Title</th>
                    <th className='p-1 p-y-2'>Price</th>
                    <th className='p-1 p-y-2'>Quantity</th>
                    <th className='p-1 p-y-2'>Total</th>
                    <th className='p-1 p-y-2'>Remove</th>
                </tr>
            </thead>
            <tbody>
                {all_products.map((e) => {
                    if (cartItems[e.id] > 0) {
                        return <tr key={e.id} className='border-b border-slate-900/20 text-gray-30 p-6 medium-14 text-center'>
                            ...
                            className='rounded-lg ring-1 ring-offset-slate-900/5 my-1' /></td>
